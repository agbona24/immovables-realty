...
interface PrimeHavenEstateClientProps {
  estate: Estate;
}

export default function PrimeHavenEstateClient({ estate }: PrimeHavenEstateClientProps) {
  ...
  return (
    <main className="min-h-screen">
      <PageHero
        ...
        breadcrumbs={[
          { name: "Home", href: "/" },
          { name: estate.name, href: "/prime-haven-estate" },
        ]}
      />
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  {estate.priceList.map((plot, index) => (
                    <div key={index} className="bg-white/10 backdrop-blur-sm rounded-xl p-6 text-center">
                      ...
                      <div className="text-blue-100 mb-3">sqm</div>
                      <div className="text-2xl font-bold text-brand-orange">{plot.price}</div>
                    </div>
                  ))}
                </div>
            ...
            {estate.paymentPlans && estate.paymentPlans.length > 0 && (
              ...
              >
                ...
                <p className="text-orange-100 mb-6 text-lg">
                  {estate.name} allows clients to secure their preferred plot with ease.
                </p>
                <div className="grid md:grid-cols-2 gap-6">
                  {estate.paymentPlans.map((plan, index) => (
                    <div key={index} className="bg-white/10 backdrop-blur-sm rounded-xl p-6">
                      ...
                      <div className="text-orange-100 mb-3">{plan.duration}</div>
                      <div className="text-sm text-orange-50">{plan.details}</div>
                    </div>
                  ))}
                </div>
              </motion.div>
